...
def render_global_filter_widgets(data):
    ...
    for col in filter_cols:
        if col in numeric_columns:
            ...
            # FIX: Check if min and max values are the same before creating a slider to prevent crashing.
            if min_val < max_val:
            ...
            # FIX: Convert to string to handle mixed types before finding and sorting unique values.
...
def render_local_filter_widgets(data, chart_key):
    ...
    with st.sidebar.expander(f"Chart-Specific Filters for: {chart_key}", expanded=False):
        ...
        for col in filter_cols:
            if col in numeric_columns:
                min_val, max_val = float(filtered_data[col].min()), float(filtered_data[col].max())
                # FIX: Check if min and max values are the same before creating a slider.
                if min_val < max_val:
                ...
                # FIX: Convert to string to handle mixed types before sorting.
                unique_vals = sorted(filtered_data[col].astype(str).unique())
