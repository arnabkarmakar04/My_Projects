...
if uploaded_file is not None:
    ...
    if st.session_state.get("original_data") is not None and not st.session_state.original_data.empty:
        ...
        st.subheader("💬 Ask Your Data",
                     help="Use this query feature for the basic purposes. For advance visualization and analysis use the build-in sections")
        st.markdown("Use Short & Precise Query in Natural Language.")
        ...
        if submitted and query_text:
            ...
            if cleaned_query in ["undo", "undo the last change", "undo last change"]:
                ...
                with st.spinner("🤖 Generating and executing code..."):
                    ...
                    prompt = f"""
                    You are a code-generation expert for the pandas library in Python.
                    ...
                    Your task is to generate a single line of Python code using the `df` DataFrame to answer this question.
                    
                    RULES:
                    - Respond with ONLY the Python code. Do not include any explanations, comments, or the word "python".
                    - The code must be a single line that can be executed.
                    - The code MUST return a value or a plotly express figure object that can be displayed.
                    - DO NOT use the `print()` function.
                    - Do not use `inplace=True` in any operations.
                    - Do not assign the result to a variable.
                    - For plotting, use plotly express (px). For example: px.bar(df, x='col1', y='col2')
                    
                    Example Question: What is the average fare?
                    Example Response: df['Fare'].mean()

                    Example Question: Plot a bar chart of the 'Sex' column
                    Example Response: px.bar(df['Sex'].value_counts())
                    """
                    ...
                    if generated_code:
                        ...
                        if is_safe:
                            try:
                                result = eval(generated_code, {'df': df, 'pd': pd, 'px': px})
                                ...
                                st.session_state.last_ai_result = str(e)
                                st.session_state.last_query = query_text
                        else:
